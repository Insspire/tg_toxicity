...
st.title("🛡️ Простая проверка токсичности")
                ...
                with TelegramClient('session_simple', API_ID, API_HASH) as client:
                    ...
                    for msg in messages:
                        if msg.replies:
                            ...
                            for reply in client.iter_messages(entity, reply_to=msg.id, limit=10):
                                if reply.message:
                    ...
                    if not comments_list:
                        ...
                        st.info(f"Собрано {len(comments_list)} комментариев. Анализируем...")
                        
                        results = classifier(comments_list)
                        
                        bad_messages = []
                        for i, res in enumerate(results):
                            # Если модель говорит, что это не 'non-toxic'
